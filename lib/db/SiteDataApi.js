...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  if (
    ...
    for (const viewId of viewIds) {
      const rawCollectionView =
        collectionView?.[viewId]?.value?.value ||
        collectionView?.[viewId]?.value ||
        collectionView?.[viewId]

      try {
        const data = await notionAPI.__call(
          'getCollectionData',
          collectionId,
          viewId,
          rawCollectionView,
        ...
        collectionQuery[collectionId][viewId] = data?.result?.reducerResults
      } catch (error) {
        console.warn('[collectionQuery] root collection fetch failed:', viewId, error)
      }
    }
  }
