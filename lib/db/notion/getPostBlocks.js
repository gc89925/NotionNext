...
export function formatNotionBlock(block) {
  ...
  const validBlockIds = new Set(
    Object.keys(clonedBlock).filter(id => clonedBlock[id]?.value?.id)
  )
  for (const blockId of Object.keys(clonedBlock)) {
    const content = clonedBlock[blockId]?.value?.content
    if (Array.isArray(content)) {
      clonedBlock[blockId].value.content = content.filter(id =>
